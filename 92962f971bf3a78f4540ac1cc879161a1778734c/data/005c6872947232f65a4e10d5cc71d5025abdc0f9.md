...
- Name: suite/e2e/dashboard/member-detail.journey.spec.ts >> Settings — team member detail >> an invalid member id shows the error state (not the detail)
- Location: suite/e2e/dashboard/member-detail.journey.spec.ts:50:7
...
```
Error: an unknown member id must surface the error state

expect(locator).toBeVisible() failed

Locator: getByTestId('member-detail-error')
Expected: visible
Error: element(s) not found

Call log:
  - an unknown member id must surface the error state with timeout 30000ms
  - waiting for getByTestId('member-detail-error')
